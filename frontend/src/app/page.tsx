...
import { 
  Droplet, Zap, Tv, Sparkles, Shield, Clock, ShieldCheck, 
...
export default function HomePage() {
  ...
  const categories = [
    { name: 'Plumbing & Leak Repairs', icon: Droplet, count: '14 Services', slug: 'plumbing', desc: 'Water tanks, pipes & toilets' },
    { name: 'Electrical & Wiring', icon: Zap, count: '9 Services', slug: 'electrical', desc: 'Short circuit diagnosis & wiring' },
    { name: 'AC & Appliance Repair', icon: Tv, count: '12 Services', slug: 'ac-appliance-repair', desc: 'AC, fridge, geysers & washing machines' },
    { name: 'Deep Home Cleaning', icon: Sparkles, count: '8 Services', slug: 'cleaning-pest-control', desc: 'Sofa cleaning & full home scrubbing' },
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      {/* Main Navbar */}
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-100 px-6 py-4 flex items-center justify-between shadow-xs">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="font-heading font-extrabold text-lg text-[#0B3C5D] tracking-tight">ServeNep</h1>
            <p className="text-[10px] text-slate-500 uppercase tracking-widest font-semibold">Home Marketplace</p>
          </div>
        </div>

        <nav className="hidden md:flex items-center gap-6 text-sm font-semibold text-slate-600">
          <Link href="#services" className="hover:text-[#0B3C5D] transition-colors">Services</Link>
          <Link href="#how-it-works" className="hover:text-[#0B3C5D] transition-colors">How It Works</Link>
          <Link href="#reviews" className="hover:text-[#0B3C5D] transition-colors">Testimonials</Link>
        </nav>

        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 text-sm bg-slate-100 border px-3 py-1.5 rounded-xl text-slate-700">
      ...
      <section className="relative gradient-hero text-white py-24 px-6 md:px-12 flex flex-col items-center text-center overflow-hidden">
        ...
        <span className="bg-white/10 text-white border border-white/20 px-3.5 py-1.5 rounded-full text-xs font-semibold uppercase tracking-wider mb-6">
        ...
        <div className="w-full max-w-2xl bg-white p-2 rounded-2xl shadow-xl flex flex-col md:flex-row gap-2 border border-slate-200/50 mb-6">
          <div className="flex-1 flex items-center gap-2 px-3 text-slate-500">
          ...
          <Link href="/booking" className="btn-secondary flex items-center justify-center gap-2 py-4">
            <span>Book Now</span>
            <ArrowRight size={16} />
          </Link>
        </div>
        ...
        <div className="flex items-center gap-3 text-sm">
          ...
          <Link href="/booking?emergency=true" className="underline hover:text-red-300 font-semibold transition-colors">
      ...
      <section className="bg-white border-y border-slate-100 py-8 px-6">
        <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          <div className="flex flex-col items-center">
            <div className="w-12 h-12 bg-slate-50 text-[#0B3C5D] rounded-full flex items-center justify-center mb-3">
              <ShieldCheck size={24} />
            </div>
            <h4 className="font-heading font-bold text-sm text-slate-800">Verified Pros</h4>
          ...
          <div className="flex flex-col items-center">
            ...
            <h4 className="font-heading font-bold text-sm text-slate-800">Service Warranty</h4>
            <p className="text-xs text-slate-500">7-Day free callback warranty</p>
          </div>
          <div className="flex flex-col items-center">
            <div className="w-12 h-12 bg-slate-50 text-[#0B3C5D] rounded-full flex items-center justify-center mb-3">
              <Clock size={24} />
            </div>
            <h4 className="font-heading font-bold text-sm text-slate-800">Quick Response</h4>
            <p className="text-xs text-slate-500">On-time technician arrival</p>
          </div>
          <div className="flex flex-col items-center">
            <div className="w-12 h-12 bg-slate-50 text-[#0B3C5D] rounded-full flex items-center justify-center mb-3">
              <Shield size={24} />
            </div>
            <h4 className="font-heading font-bold text-sm text-slate-800">Transparent Pricing</h4>
            <p className="text-xs text-slate-500">Upfront estimates, no tricks</p>
          </div>
        </div>
      </section>
      ...
      <section id="services" className="py-20 px-6 max-w-6xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="font-heading text-3xl md:text-4xl font-black text-[#0B3C5D] mb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {categories.map((cat, i) => (
            <Link 
              ...
              className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-xs hover:shadow-md hover:border-sky-500/40 transition-all group flex flex-col"
            >
              <div className="w-12 h-12 rounded-xl bg-slate-50 text-[#328CC1] group-hover:bg-[#328CC1] group-hover:text-white transition-all flex items-center justify-center mb-6">
                <cat.icon size={24} />
              </div>
              <h3 className="font-heading font-bold text-slate-800 text-lg group-hover:text-primary transition-colors mb-1">{cat.name}</h3>
      ...
      <section id="reviews" className="py-20 px-6 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            <div key={idx} className="bg-white p-6 rounded-2xl border border-slate-200/50 shadow-xs flex flex-col justify-between">
              <div>
              ...
              <div>
                <h4 className="font-heading font-bold text-slate-800 text-sm">{t.name}</h4>
      ...
      {/* Footer */}
      <footer className="mt-auto bg-slate-900 border-t border-slate-800 text-slate-400 py-12 px-6 text-sm">
        ...
        <div className="max-w-6xl mx-auto border-t border-slate-800 pt-8 flex flex-col md:flex-row items-center justify-between text-xs text-slate-500 gap-4">
