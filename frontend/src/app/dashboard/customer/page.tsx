        ...
        <main className="flex-1 max-w-6xl w-full mx-auto p-6 md:p-12 grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            <div className="bg-white p-6 rounded-2xl border border-slate-200/50 shadow-xs text-center space-y-4">
              ...
              <div>
                ...
                <span className="inline-block mt-2 text-[10px] bg-slate-100 text-slate-500 font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
                  {user?.role}
                </span>
              </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200/50 shadow-xs space-y-4">
              ...
              {profile?.addresses?.length > 0 ? profile.addresses.map((addr: any) => (
                <div key={addr.id} className="p-3 rounded-xl border border-slate-100 text-xs flex items-start gap-2.5">
                  <MapPin size={16} className="text-[#328CC1] mt-0.5 shrink-0" />
                  <div>
                    <h5 className="font-bold text-slate-800">{addr.label}</h5>
                    <p className="text-slate-500 mt-0.5">{addr.street}, {addr.city}</p>
                  </div>
                </div>
              )) : (
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="flex justify-between items-center">
              <h2 className="font-heading text-xl font-extrabold text-[#0B3C5D]">Your Booking Orders</h2>
            ...
            {bookings.length === 0 ? (
              ...
              <div className="space-y-4">
                {bookings.map((booking) => (
                  <div key={booking.id} className="bg-white p-6 rounded-2xl border border-slate-200/50 shadow-xs space-y-4 hover:shadow-md transition-shadow">
                    <div className="flex justify-between items-start">
                      <div>
                        <span className="text-slate-400 text-xs font-extrabold tracking-wider">{booking.bookingNumber}</span>
                        <h4 className="font-heading font-bold text-slate-800 text-base mt-0.5">{booking.itemName}</h4>
                      </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs font-semibold text-slate-500 border-y py-3">
                      <div>
                        <p className="text-xs text-slate-400 uppercase">Pro Assigned</p>
                        <p className="text-slate-800 font-bold mt-0.5">{booking.technicianName || 'Searching...'}</p>
                      </div>
                      <div>
                        <p className="text-xs text-slate-400 uppercase">Scheduled</p>
                        <p className="text-slate-800 mt-0.5">{booking.scheduledDate}</p>
                      </div>
                      <div>
                        <p className="text-xs text-slate-400 uppercase">Payment</p>
                        <p className="text-slate-800 font-bold mt-0.5">{booking.paymentStatus}</p>
                      </div>
                      <div>
                        <p className="text-xs text-slate-400 uppercase">Amount</p>
                        <p className="text-slate-800 font-bold mt-0.5">Rs. {booking.totalAmount}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-3 justify-end text-xs font-bold pt-1">
                      ...
                      <a href={`http://localhost:5000/api/v1/bookings/${booking.id}`} target="_blank" rel="noopener noreferrer" className="btn-primary py-2 flex items-center gap-1">
        ...
        {showReviewModal && (
          <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-white p-6 rounded-2xl max-w-sm w-full space-y-4 shadow-xl">
              <h3 className="font-heading font-extrabold text-slate-800 text-lg">Leave a Review</h3>
              <div className="space-y-2">
                <label className="block text-xs font-bold text-slate-500 uppercase">Rating</label>
              ...
              <div className="space-y-2">
                <label className="block text-xs font-bold text-slate-500 uppercase">Comment</label>
                <textarea rows={3} value={comment} onChange={(e) => setComment(e.target.value)} placeholder="How was your technician? Upfront? Professional?" className="w-full p-3 border rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-[#328CC1]"></textarea>
              </div>
