...
export default function EsewaReturnPage() {
  ...
  useEffect(() => {
    ...
    if (params.get('failed') === 'true' || !data) {
      setState('failed');
      setMessage('The eSewa payment was cancelled or did not complete.');
      return;
    }

    fetchApi('/payments/esewa/callback', {
      method: 'POST',
      ...
      .catch((err: any) => {
        setState('failed');
        setMessage(err.message || 'Could not verify the eSewa payment.');
      });
